...
export default function AlterarCliente({ livro, communication }) {
    ...
    const Aceitar = () => {
        if (erros.isbn.valid) {
            communication.bearerPOST(localStorage.token, '/livrosDelete',
                JSON.stringify({ id: livro.id}))
                .then((retorno) => {
                    if(retorno == 'Error') {
                        alert('Não foi possível deletar o livro, ele possui empréstimos o envolvendo.');
                    }
                    setOpen(false);
                });
        }
    ...
    return (
        <div>
            ...
            <Dialog open={open} align='center' onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogTitle id="alert-dialog-title">{`Deletar ${livro.nome}`}</DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        Tem certeza que deseja apagar o livro? Todos os registros o envolvendo, irão ser apagados também.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
